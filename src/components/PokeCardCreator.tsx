  ...
  useEffect(() => {
    ...
    const renderCardReset = setInterval(() => {
      async function makeCard(inputImage: InstanceType<typeof Jimp>) {
        ...
        // add overlay
        const blank = new Jimp ({width: 726, height: 996, color: rgbaToInt(0,0,0,0)});
        blank.opacity(0);
        ...
        if (formData.weakness != "none") {
          weakness = await Jimp.read(`/poke/energy-small-${formData.weakness}.png`);
        } else {
          weakness = new Jimp({width: 1, height: 1, color: rgbaToInt(0,0,0,0)});
        }
        let resistance;
        if (formData.resistance != "none") {
          resistance = await Jimp.read(`/poke/energy-small-${formData.resistance}.png`);
        }
        else {
          resistance = new Jimp({width: 1, height: 1, color: rgbaToInt(0,0,0,0)});
        }
        ...
        card.print({font: gillRbi22 , x: 100, y: 529, text:{text:`${formData.descType} Pokemon. Length: ${lengthValue}, Weight: ${weightValue} lbs.`, alignmentX: HorizontalAlign.CENTER}, maxWidth: 525, maxHeight: 30});

        // flavor text
        card.print({font: gillRbi22 , x: 87, y: 877, text:{text:`${formData.flavorText}`, alignmentX: HorizontalAlign.LEFT}, maxWidth: 590, maxHeight: 55});
    ...
    <div className="flex gap-0 w-full h-svh justify-evenly items-center">
      ...
      <form onSubmit={handleUpdate} onChange={handleUpdate} className="flex gap-10 text-black text-2xl rounded-3xl bg-slate-700 bg-opacity-40 px-12 py-10">
        <div className="flex flex-col gap-5">
          ...
          <div>
            <label className="" htmlFor="image-select">Image</label>
            <input name="image-select" type="file" accept="image/*" onChange={handleFile}/>
          </div>
          <div className="flex gap-8">
            ...
            <div>
              <label className="" htmlFor="hp">HP</label>
          ...
          <div className="flex gap-8">
            ...
            <div className="">
              <label className="" htmlFor="type">Energy Type</label>
          ...
          <div>
            ...
            <div className="flex gap-4">
              <input type="range" name="length-feet" min="1" max="360" value={lengthSlider} onChange={handleLengthSlider} className="w-3/4"/>
          ...
          <div>
            ...
            <div className="flex gap-4">
              <input type="range" name="weight" min="1" max="592" value={weightSlider} onChange={handleWeightSlider} className="w-3/4"/>
              <span className="font-bold italic mx-auto">{weightValue} lbs</span>
            </div>
          </div>
          <div>
        ...
        <div className="flex flex-col gap-5 border-l-zinc-300 border-opacity-40 border-l-2 pl-12">
          <div className="bg-zinc-300 bg-opacity-30 rounded-xl pb-6 pt-4 px-8">
            <label className="italic text-orange-400 w-full text-center">~ First Attack ~</label>
            <div className="flex gap-8">
              <div>
                <label className="" htmlFor="move1name">Name</label>
                <input type="text" name="move1name" maxLength={12} />
              </div>
              <div>
                <label className="" htmlFor="move1dmg">Dmg</label>
          ...
          <div className="bg-zinc-300 bg-opacity-30 rounded-xl pb-6 pt-4 px-8">
            <label className="italic text-rose-400 w-full text-center">~ Second Attack ~</label>
            <div className="flex gap-8">
              <div>
                <label className="" htmlFor="move2name">Name</label>
                <input type="text" name="move2name" maxLength={12}/>
              </div>
              <div>
                <label className="" htmlFor="move2dmg">Dmg</label>
          ...
          <div className="flex gap-8">
            <div>
              <label className="" htmlFor="weakness">Weakness</label>
            ...
            <div>
              <label className="" htmlFor="resistance">Resistance</label>
          ...
          <div>
            ...
            <select name="retreat-cost" id="retreat-cost" className="text-center">
              <option value="0" className="pr-4">none</option>
              <option value="1" className="">✴️</option>
              <option value="2" className="">✴️✴️</option>
              <option value="3" className="">✴️✴️✴️</option>
            </select>
          </div>
